...
parser.add_argument('--valroot', help='path to dataset', default='/home/narcissus/file/recent_job/ocr/crnn/tool/final_test_hori_sub/')
...
parser.add_argument('--n_tes_disp', type=int, default=10, help='Number of samples to display when tes')
...
print(opt)
# tes_dataset = dataset.lmdbDataset(
#     root=opt.valroot, transform=dataset.resizeNormalize((opt.imgW, opt.imgH)))

tes_dataset = dataset.lmdbDataset(root=opt.valroot)
# tes_dataset = dataset.lmdbDataset(
...
def val(net, val_dataset, criterion, max_iter=100):
    ...
    dst_file.write(gt[0] + '_' + str(img_num) + '.jpg' + ' ' + str_line + '\n')
    dst_file.close()
val(model, tes_dataset, criterion)